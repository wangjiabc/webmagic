...
public class Spider52pojiePageProcessor implements PageProcessor {
...
*/
    ...
    public static final String URL_POST = "http://0830bbs.com/t-\\d+\\-1-1.html";
    ...
    public void process(Page page) {
        ...
        if (page.getUrl().regex(URL_LIST).match()) {
            ...
            page.putField("title", page.getHtml().xpath("//*[@id=\"postlist\"]/table/tbody/tr/td/h1/span/text()").toString());
            page.putField("content", page.getHtml().xpath("//td[contains(@id,'postmessage')]/outerHtml()").toString());
            page.putField("date",
                                page.getHtml().xpath("//em[contains(@id,'authorposton')]/text()").toString().substring(3,page.getHtml().xpath("//em[contains(@id,'authorposton')]/text()").toString().length()));
    ...
    public static void main(String[] args) {
    	...
    	Spider.create(new Spider52pojiePageProcessor()).addUrl("http://0830bbs.com/f-40-1.html")
        .addPipeline(new FilePipeline("C:\\Users\\WangJing\\Desktop\\aaaaaaaaaaaa")).thread(3).run();
    }
}